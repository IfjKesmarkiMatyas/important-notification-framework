...
class DeliveryServiceTest {
    ...
    @Test
    void enqueueAlertRendersHeadlineAndReason() {
        when(jobs.save(any())).thenAnswer(invocation -> invocation.getArgument(0));
        when(templateEngine.process(eq("alert-email"), any(Context.class))).thenReturn("<p>alert</p>");
        ...
        service.enqueue(new EnqueueDeliveryCommand(
                ...
                null,
                null,
                Map.of("headline", "6.7 Peru", "reason", "Earthquake M6.7 ≥ 6.", "url", "https://example.test", "level", "critical")
        ));

        ArgumentCaptor<DeliveryJob> captor = ArgumentCaptor.forClass(DeliveryJob.class);
        verify(jobs).save(captor.capture());
        DeliveryJob job = captor.getValue();
        assertThat(job.getSubject()).isEqualTo("6.7 Peru");
        assertThat(job.getBodyText()).contains("Earthquake").contains("https://example.test");
        assertThat(job.getBodyHtml()).isEqualTo("<p>alert</p>");
    }
